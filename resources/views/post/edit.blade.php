<html>
    ...
    <body class="grid place-items-center">
        <div class="border-2 border-black  w-1/2 h-1/2 relative p-2">
            <form action="{{ route('posts.update', ['post' => $post['id']]) }}" method="POST" class="relative flex-col flex w-full h-full">
                @csrf
                @method('PUT')
                <div class="w-full h-30 flex justify-around items-center">
                    <div class="flex space-x-3">
                        <label for="ptitle" class="text-red-500">Post Title</label>
                        <input type="text" class="outline-none bg-slate-100 p-2 rounded-md" name="ptitle" id="ptitle" value="{{ $post['posttitle'] }}">
                    </div>
                    <div class="flex space-x-3">
                        <label for="pcategory" class="text-red-500">Post Category</label>
                        <input type="text" class="outline-none bg-slate-100 p-2 rounded-md" name="pcategory" 
                        value="{{ $post['postcategory'] }}" id="pcategory">
                    </div>
                </div>
                <div class="flex-1 relative p-5">
                    <label for="pdata" class="text-red-500 underline-offset-2 underline">Post Data</label>
                    <textarea class="w-full h-full outline-none p-5" name="pdata" id="pdata">{{ $post['postdata'] }}</textarea>
                </div>
